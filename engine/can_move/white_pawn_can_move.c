    ...
    if ((board_state->occupied & position_to_u64(x, y + 1)) == 0)
    {
        ...
        if (y == 1)
        {
            if ((board_state->occupied & position_to_u64(x, y + 2)) == 0)
            {
                ...
                attacks = generate_black_attacks(&new_board_state);
                if (!(attacks & new_board_state.board.white_pieces.king))
                    return true;
            }
        }
...
{
    if (x > 0 && (board_state->black_pieces & position_to_u64(x - 1, y + 1)) != 0)
    {
        BoardState new_board_state = {0};
        copy_board(&board_state->board, &new_board_state.board);
        new_board_state.board.white_pieces.pawns &= ~position_to_u64(x, y);
    ...
    if (x < 7 && (board_state->black_pieces & position_to_u64(x + 1, y + 1)) != 0)
    {
        BoardState new_board_state = {0};
        copy_board(&board_state->board, &new_board_state.board);
        new_board_state.board.white_pieces.pawns &= ~position_to_u64(x, y);
