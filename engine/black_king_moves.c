...
static inline void generate_black_king_moves(BoardState *board_state, uint8_t x, uint8_t y, BoardStack *stack)
{
    for (int dy = -1; dy <= 1; dy++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            {
                if (board_state->white_pieces & position_to_u64(x + dx, y + dy))
                {
                    BoardState *new_board_state = &stack->boards[stack->count];
                    copy_board(&board_state->board, &new_board_state->board);
                    remove_white_piece(new_board_state, x + dx, y + dy);
                    new_board_state->board.black_pieces.king &= ~position_to_u64(x, y);
                    new_board_state->board.black_pieces.king |= position_to_u64(x + dx, y + dy);
